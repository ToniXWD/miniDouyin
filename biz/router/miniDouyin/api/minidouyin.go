...
import (
	...
	api "miniDouyin/biz/handler/miniDouyin/api"
)
	...
	root := r.Group("/", rootMw()...)
	{
		...
		}
		{
			_message := _douyin.Group("/message", _messageMw()...)
			{
				...
				_action1.POST("/", append(_sendmsgMw(), api.SendMsg)...)
			}
			{
				_chat := _message.Group("/chat", _chatMw()...)
				_chat.GET("/", append(_chatrecMw(), api.ChatRec)...)
			}
		}
		{
			_publish := _douyin.Group("/publish", _publishMw()...)
			{
				...
				_action2.POST("/", append(_videopublishactionMw(), api.VideoPublishAction)...)
			}
			{
				_list0 := _publish.Group("/list", _list0Mw()...)
				_list0.GET("/", append(_publishlistMw(), api.PublishList)...)
